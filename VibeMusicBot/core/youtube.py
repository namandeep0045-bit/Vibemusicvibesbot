...
from VibeMusicBot.helpers import Track, utils
...
class YouTube:
    ...
    def get_cookies(self):
        if not self.checked:
            try:
                for file in os.listdir("VibeMusicBot/cookies"):
        ...
        if not self.cookies:
            ...
            return None
        return f"VibeMusicBot/cookies/{random.choice(self.cookies)}"
    ...
    async def save_cookies(self, urls: list[str]) -> None:
        ...
        for url in urls:
            try:
                path = f"VibeMusicBot/cookies/cookie{random.randint(10000, 99999)}.txt"
            ...
            def _download(ydl_runtime_opts):
                ...
                try:
                    ...
                    if "unable to rename file" in error_msg.lower() and recovered:
                        logger.warning(f"⚠️ Renaming failed for {video_id}, using recovered file {Path(recovered).name}")
                        return recovered
                    if "416" in error_msg or "Requested range not satisfiable" in error_msg:
                        ...
                        if recovered:
                            logger.warning(f"⚠️ Using recovered file for {video_id} despite download error")
                            return recovered
                    return None
                except Exception as ex:
